...
I chose the magma palette, due to the changes in median income being boldly reflected in the color scale. The colors visually denote changes in income the best. 
...
Changing the classification changes the visual distribution of the median income. When utilizing the step_quantile() function, the amount of instances per category are equivalent, thus is why we see quite a bit of homes in both the lowest and highest categories. Also, the amount between ranges are not equal to each other. This is used when you want equal amounts of instances per group. Now when using the step_equal_interval() function, there is not an equal amount of instances per category, but the amount between ranges is equivalent. When using step_quantile(), each color is visually represented evenly, but when using step_equal_interval(), there are less instances of certain colors, but the ranges are balanced. Making a decision is based on if you want the instances or the categories represented. 
...
I chose the step_equal_interval() because I wanted the map to have the ranges be equal to each other, which highlights the differences between groups. Although this hides outliers, I wanted to show numeric differences are clearly rather than balancing the color spread. Also, I think that the categorical styling better differentiates the income groups visually, but perhaps the continuous styling better portrays a scale of which a home stands. For example, the categorical coloring is simple to denote, but if a home is on the edge of the range, it could be grouped into a broad range and be misleading. 
...
hi_median_income <- hi_median_income |>
  mutate(
    poverty_flag = ifelse(estimate < 30650, "Below Poverty Line", "Above Poverty Line")
  )
```
...
hi_median_income <- hi_median_income |>
  mutate(
    income_flag = ifelse(estimate < 62233, "Below $62,233", "Above $62,233")
  )
```
...
income_colors = c("Above $62,233" = "green", "Below $62,233" = "orange")
...
mapboxgl(bounds = hi_median_income) |>
  add_fill_layer(
    ...
    fill_color = match_expr(
      column = "income_flag",
      values = names(income_colors),
      stops = unname(income_colors)
    ),
    fill_opacity = 0.6
  ) |>
  add_legend(
...
hi_median_income$tooltip <- glue(
  "{hi_median_income$NAME}\nMedian Income: ${hi_median_income$estimate}"
)
...
# 🚜 NORTH DAKOTA MAP - Per Capita Income
```{r}

# Per Capita Income in ND
nd_income <- get_acs(
  geography = "tract",
...
mapboxgl(bounds = nd_income) |>
  add_fill_layer(
    id = "nd_tracts",
    source = nd_income,
    fill_color = nd_scale$expression,
    fill_opacity = 0.5
  ) |>
  add_legend(
    "Per Capita Income in North Dakota",
    values = get_legend_labels(nd_scale, digits = 0, prefix = "$"),
    colors = get_legend_colors(nd_scale),
    type = "categorical"
  )
